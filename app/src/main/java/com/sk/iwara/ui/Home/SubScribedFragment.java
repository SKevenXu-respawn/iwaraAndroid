...
public class SubScribedFragment extends BaseFragment<FragmentSubscribedBinding> {
    ...
    protected void init() {
        map=new HashMap<>();
        map.put("Authorization","Bearer "+ LoginSPUtil.getInstance(getContext()).get("access_token",null));
    ...
    protected void initUI() {
        ...
        binding.subscribedRecyle.setLayoutManager(new GridLayoutManager(getContext(), 2));
        binding.subscribedRecyle.setAdapter(adapter);
        binding.subscribedRecyle.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void getData(){
        ...
        HttpUtil.get().getAsync(IWARA_API.VIDEO+"/videos?rating=all&limit=20&subscribed=true&page=0", null,map, new HttpUtil.NetCallback() {
            @Override
    ...
    private void loadMore() {
        ...
        HttpUtil.get().getAsync(IWARA_API.VIDEO+"/videos?rating=all&limit=20&subscribed=true&page="+(++page), null,map,
